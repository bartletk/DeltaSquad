<?php
		...
		if(!$result || (mysql_num_rows($result) < 1)){
			...
			while($row = mysql_fetch_assoc($result)) {
				...
				if ($row['status'] != 'approved') {
					$style = "style='color: red; text-shadow: 1px 1px 2px black, 0 0 25px orange; 0 0 5px black;'";
				...
				echo "<div ".$class.">";
				echo "<a href='/showevent.php?e=".$row['event_id']."&s=".$row['series']."$studentinfo' $style>".$cnum." ".$title."</a>";
				if ($row['series'] != 9100){
					...
					if ($session->isAdmin() && $cm == 1){
						echo "<br><a href='./viewconflict.php?e=".$row['event_id']."'>Conflict Manager</a>";
						}
				}
				echo "</div><hr>";
			}
	...
	function showMonth ($calmonth,$calyear) {
		global $week_titles, $o, $m, $a, $y, $w, $c, $next, $prev,$ly, $lm, $le, $la, $sem, $cwid, $studentCWID, $rm, $cm;
		...
		echo '<th colspan="7" class="cal_top"><a href="index.php?o=',$o,'&w=',$w,'&c=',$c,'&m=',$prev["month"]["m"],'&a=1&y=',$prev["month"]["y"],'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">&lt;</a> ',date('F', mktime(0,0,0,$calmonth,1,$calyear)),'&nbsp;',date('Y', mktime(0,0,0,$calmonth,1,$calyear)),' <a href="index.php?o=',$o,'&w=',$w,'&c=',$c,'&m=',$next["month"]["m"],'&a=1&y=',$next["month"]["y"],'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">&gt;</a></th></tr><tr>';
		for ( $x = 0; $x < 7; $x++ ){
		...
		for ($t=0; $t < $offset; $t++) {
			if ($t == 0) {
				...
				echo '<td class="day noshow"><div class="week"><a href="index.php?o=',$le,'&w=',$w,'&c=',$c,'&m=',$offmonth,'&a=',$offday,'&y=',$offyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">week</a></div></td>';
				} else {
				if ($offset == 6 || $offset == 0){
		...
		for ( $d = 1; $d <= $totaldays; $d++ )
		{
			if (($d == date('j')) && ($calmonth == date('m')) && ($calyear == date('Y'))) {
				...
				if ($offset == 0 || $offset == 6){
					echo '<td class="day noshow" id="today"><div class="day_of_month"><a href="index.php?o=',$la,'&w=',$w,'&c=',$c,'&m=',$calmonth,'&a=',$d,'&y=',$calyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">', $d, '</a></div>';
					
					} else {
					echo '<td class="day" id="today"><div class="day_of_month"><a href="index.php?o=',$la,'&w=',$w,'&c=',$c,'&m=',$calmonth,'&a=',$d,'&y=',$calyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">', $d, '</a></div>';
				}
				} else {
				if ($offset == 0 || $offset == 6){
					echo '<td class="day noshow"><div class="day_of_month"><a href="index.php?o=',$la,'&w=',$w,'&c=',$c,'&m=',$calmonth,'&a=',$d,'&y=',$calyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">', $d, '</a></div>';
					} else {
					echo '<td class="day"><div class="day_of_month"><a href="index.php?o=',$la,'&w=',$w,'&c=',$c,'&m=',$calmonth,'&a=',$d,'&y=',$calyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">', $d, '</a></div>';
				}
				if ($offset == 0) echo '<div class="week"><a href="index.php?o=',$le,'&w=',$w,'&c=',$c,'&m=',$calmonth,'&a=',$d,'&y=',$calyear,'&sem=',$sem,'&cwid=',$studentCWID,'&rm=',$rm,'&cm=',$cm,'">week</a></div>';
