...
<html>
			...
			*/
			...
			<main>
				...
				<div>
					...
					<textarea rows="20">
						<?php
							$q = "SELECT user.name, log.* FROM ".TBL_LOG." join ".TBL_USERS." WHERE user.CWID = log.CWID";
					...
					<form action="adminprocess.php" method="POST" id="archive">
						...
						<button class="btn waves-effect waves-light" type="submit" >Archive & Reset
							<i class="material-icons right">send</i>
						</button>
					</form>
	...
	foreach ($rss->items as $item) {
		...
		$time = time();
		$fillRSS = sprintf("INSERT INTO ".TBL_EVENTS." VALUES (NULL, '$title', 0, 'event', 0, 86753091, 'offsite', '$notes', 9100, '$date', '$date', '$time', 'approved')");
		$result = $database->query($fillRSS);
		//$myfile = fopen("error.txt", "a") or die(print_r($fillRSS));
