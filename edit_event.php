...
if (!$superview) {
	...
	if (!isset($_POST["mode"])) {
		...
		$q = "SELECT * from dates where event_id =".$_REQUEST["id"];
		$query = mysql_query($q);
		if (!$query) $msg .= "Database Error : ".$q;
		else {
			$i = 0;
	...
	if ($_POST["mode"] == "Update Event") {
		...
		$q = "UPDATE events set title = '".$title."', venue_id = ".$link_id.", contact_id = ".$contact_id.", description = '".$description."', category_id = ".$category_id.", group_id = ".$group_id.", status_id = ".$new_status_id." where event_id = ".$_POST["id"];
		$query = mysql_query($q);
		if (!$query) $msg .= "Database Error : ".$q;
		else {
			$q = "delete from dates where event_id = ".$_POST["id"];
			$query = mysql_query($q);
			if (!$query) $msg .= "Database Error : ".$q;
			$j = 0;
			while ($_POST["date_".$j]) {
							...
							if (($_POST["end_time_".$j])&&(!preg_match("/([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)/i",$_POST["end_time_".$j],$end_time))) {
								...
								$end_date = $date[3]."-".$date[1]."-".$date[2]." ".$end_time[1].":".$end_time[2];
									if (($end_time[1].$end_time[2]) >= ($start_time[1].$start_time[2])) {
										$q = "INSERT into dates (event_id, date, end_date) values (".$_REQUEST["id"].", '".$start_date."', '".$end_date."')";
									} else {
										$q = "INSERT into dates (event_id, date) values (".$_REQUEST["id"].", '".$start_date."')";
									}
								} else {
									$q = "INSERT into dates (event_id, date) values (".$_REQUEST["id"].", '".$start_date."')";
								}
						...
						if (($_POST["end_time_add"])&&(!preg_match("/([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)/i",$_POST["end_time_add"],$end_time))) {
							...
							while ($juno <= $r) {
								...
								if ($_POST["end_time_add"]) {
									$end_date = $date[3]."-".$date[1]."-".$date[2]." ".$end_time[1].":".$end_time[2];
									if (($end_time[1].$end_time[2]) >= ($start_time[1].$start_time[2])) {
											$q = "INSERT into dates (event_id, date, end_date) values (".$_REQUEST["id"].", DATE_ADD('".$start_date."', INTERVAL ".$next." DAY), DATE_ADD('".$end_date."', INTERVAL ".$next." DAY))";
										} else {
											$q = "INSERT into dates (event_id, date) values (".$_REQUEST["id"].", DATE_ADD('".$start_date."', INTERVAL ".$next." DAY))";
										}
								} else {
									$q = "INSERT into dates (event_id, date) values (".$_REQUEST["id"].", DATE_ADD('".$start_date."', INTERVAL ".$next." DAY))";
								}
...
if ($_POST["date_add"]) {
	...
	//loop through recurring events
	preg_match("/([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})/i",$_POST["date_add"],$date);
	$juno = 0;
	$r = $_POST["recurring"];
	if (!$r) $r = 0;
	$int = $_POST["interval"];
