			...
			<?php
				...
				<form action="process.php" method="POST" class="col s12" id="addevent">
					...
					<div class="row col s12" >
						<input type="date" placeholder="Date" class = "datepicker" name="date">
					...
					-->
						...
						<div class="input-field col s12">
    <select multiple>
        <option id="repeat" name="repeat" maxlength="30" value="1" <?php if (isset($_GET['repeat']) && $_GET['repeat']== 1){echo "checked";}?> />Check this and days you want to repeat</option>
        <option name="repeatm" maxlength="30" value="1" <?php if (isset($_GET['repeatm']) && $_GET['repeatm'] == 1){echo "checked";}?> id="repeatm" />Monday</option>
	<option name="repeatt" maxlength="30" value="1" <?php if (isset($_GET['repeatt']) && $_GET['repeatt']== 1){echo "checked";}?> id="repeatt" />Tuesday</option>
        <option value="1" name="repeatw" maxlength="30" value="1" <?php if (isset($_GET['repeatw']) && $_GET['repeatw'] == 1){echo "checked";}?> id="repeatw" />Wednesday</option>
        <option value="1" name="repeatth" maxlength="30" value="1" <?php if (isset($_GET['repeatth']) && $_GET['repeatth'] == 1){echo "checked";}?> id="repeatth" />Thursday</option>
	<option value="1" name="repeatf" maxlength="30" value="1" <?php if (isset($_GET['repeatf']) && $_GET['repeatf'] == 1){echo "checked";}?> id="repeatf" />Friday</option>
    </select>
    <label>Repeat</label>
					...
					<form action="process.php" method="POST" id="addeventC">
						<div class="fudge">
							<div class="input-field col s12">
								...
								$q2 = "SELECT DISTINCT room_number, description, (SELECT count(*) FROM ".TBL_ROOMS." r, ".TBL_EVENTS." e, WHERE ((e.dateStart < STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s') AND e.dateEnd > STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s')) OR (e.dateStart < STR_TO_DATE('$datetimeStart', '%Y-%m-%d %H:%i:%s') AND e.dateEnd > STR_TO_DATE('$datetimeStart', '%Y-%m-%d %H:%i:%s'))) OR (e.dateStart > STR_TO_DATE('$datetimeStart', '%Y-%m-%d %H:%i:%s') AND e.dateEnd < STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s'))) AND r.room_number = e.room_number AND series != 9100) AS failures FROM ".TBL_ROOMS." r, ".TBL_EVENTS." e WHERE capacity >= $capacity AND EXISTS (SELECT * FROM ".TBL_ROOMS." r, ".TBL_EVENTS." e, WHERE ((e.dateStart < STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s') AND e.dateEnd > STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s')) OR (e.dateStart < STR_TO_DATE('$dateStart', '%Y-%m-%d %H:%i:%s') AND e.dateEnd > STR_TO_DATE('$dateStart', '%Y-%m-%d %H:%i:%s'))) OR (e.dateStart > STR_TO_DATE('$dateStart', '%Y-%m-%d %H:%i:%s') AND e.dateEnd < STR_TO_DATE('$datetimeEnd', '%Y-%m-%d %H:%i:%s'))) AND r.room_number = e.room_number AND series != 9100) GROUP BY failures ORDER BY failures";
