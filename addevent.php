		...
		<div class="row">
			<?php
				...
				<form action="process.php" method="POST" class="col s12" id="addevent">
					...
					<div>
						<a class="waves-effect waves-light btn modal-trigger" href="#modal1">Click if You want to Repeat this Event</a>
						
						<div id="modal1" class="modal">
    						<div class="modal-content">
								<p style="text-align:center; color:#600;">
					...
					<!--dropdown to course-->
					<div class="input-field col s12">
						<select form="addevent" name="course" required value="<?php if (isset($_GET['c'])) echo $_GET['c']?>">
					...
					<div class="card card-2">
						<p><b>Title:</b> <?php echo $_GET['t']; ?> </p>
					
					
						<p><b>Date:</b> <?php echo date('F d, Y', strtotime($_GET['d'])); ?></p>
					
					
						<p><b>Start time:</b> <?php echo $_GET['st']; ?></p>
					
						<p><b>End time:</b> <?php echo $_GET['et']; ?></p>
					
						<p><b>Attendees:</b> <?php echo $_GET['s']; ?></p>
					
						<p><b>Notes:</b> <?php echo $_GET['n']; ?></p>
					
						<p><b>Course: NURS</b> <?php echo $_GET['c']; ?></p>
					</div>	
						...
						<div class="outter">
							<div class="input-field col s12">
								<?php
									...
									makeTable('Offsite', 'Any offsite location.', 'Put location info in notes.', 0, $date, 0);
									...
									if ($num_rows > 0){
										for($l=0; $l<$num_rows4; $l++){
											...
											$msg = $conflicts." conflicts and capacity: ".$capacity;
											$date = date("Y-m-d",strtotime($datetimeStart));
							...
							<?php
								...
								if ($tail == 0 && !$session->isAdmin()){
									echo '<input type="hidden" name="tail" value="0">';
								}
								 else if ($tail != 0 || $session->isAdmin()){
									echo '<input type="hidden" name="tail" value="1">';
								}
							?>
				...
				function makeTable($num, $desc, $msg, $bg, $date, $conflict){
					...
					echo '<a class="waves-effect waves-light btn modal-trigger" href="#modal'.$num.'" style="width:98%;margin-left:1%;margin-bottom:1%;">View Schedule</a>
					...
					</table>'.showRoomGrid($date, $num).'</div></div>';
				...
				function showRoomGrid($date, $room) {
					...
					if(!$result || (mysql_num_rows($result) < 1)){
						// NO EVENTS
						echo "<div class=\"wrap\"><div class=\"date\" style=\"border:none;";
			...
			<script>
				...
				function testTime(){
					var start = "<?php echo $_GET['st']; ?>";
					var end = "<?php echo $_GET['et']; ?>"
					if (end < start)
					{
